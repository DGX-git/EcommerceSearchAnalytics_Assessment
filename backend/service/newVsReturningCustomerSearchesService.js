...
// backend/service/newVsReturningCustomerSearchesService.js
const models = require('../models');
...
const newVsReturningCustomerSearchesService = async (request, response) => {
  try {
    ...
    // Fetch all searches with customer data (optionally filtered by date)
    const searches = await models.Search.findAll({
      attributes: ['customer_id', 'search_id', 'min_rating', 'total_results', 'search_date'],
      where,
      raw: true
    });
    ...
    const result = Object.entries(segments)
      ...
      .map(([type, data]) => ({
        ...
        total_searches: data.total_searches,
        avg_searches_per_customer: parseFloat((data.total_searches / data.unique_customers).toFixed(2)),
        avg_rating: data.count > 0 ? parseFloat((data.avg_rating_sum / data.count).toFixed(2)) : 0,
        avg_results: data.count > 0 ? parseFloat((data.avg_results_sum / data.count).toFixed(2)) : 0
      }))
      .sort((a, b) => b.total_searches - a.total_searches);
