...
// backend/service/crossSearchPatternsService.js
const models = require('../models');
 ...
 * Service to identify sequential search patterns (keyword sequences)
 * Finds patterns like "serum" followed by "vitamin C" within a customer's search history
 * Supports date filtering via ?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD
 */
const crossSearchPatternsService = async (request, response) => {
  try {
    ...
    // Fetch searches ordered by customer and date, with optional date filter
    const searches = await models.Search.findAll({
      attributes: ['search_id', 'customer_id', 'search_keyword', 'search_date'],
      where,
      order: [['customer_id', 'ASC'], ['search_date', 'ASC']],
    ...
    // Filter and prepare result
    const result = Object.values(patterns)
      .filter(p => p.frequency >= 1) // keep patterns with at least 1 occurrence (adjust threshold as needed)
      .map(p => ({
        pattern: p.pattern,
        frequency: p.frequency,
        user_count: p.user_ids.size,
        pattern_percentage: totalSearches > 0 ? parseFloat(((p.frequency * 100.0 / totalSearches).toFixed(2))) : 0
      }))
      .sort((a, b) => b.frequency - a.frequency)
      .slice(0, 200);
